  ...
  for (i = 0; i < pum_height; ++i) {
    ...
    if (curwin->w_p_rl) {
      if (pum_col < curwin->w_wincol + curwin->w_width - 1) {
        screen_putchar(NULL, ' ', row, pum_col + 1, attr);
      }
    } else if (pum_col > 0) {
      screen_putchar(NULL, ' ', row, pum_col - 1, attr);
    }
    ...
    for (round = 1; round <= 3; ++round) {
          ...
          if ((*p == NUL) || (*p == TAB) || (totwidth + w > pum_width)) {
            ...
            *p = saved;
            ...
            if (curwin->w_p_rl) {
              ...
              screen_puts_len(NULL, rt, (int)STRLEN(rt), row, col - size + 1,
                              attr);
              xfree(rt_start);
              xfree(st);
              col -= width;
            } else {
              screen_puts_len(NULL, st, (int)STRLEN(st), row, col, attr);
              xfree(st);
              col += width;
            }
            ...
            if (curwin->w_p_rl) {
              screen_puts_len(NULL, (char_u *)"  ", 2, row, col - 1, attr);
              col -= 2;
            } else {
              screen_puts_len(NULL, (char_u *)"  ", 2, row, col, attr);
              col += 2;
            }
            totwidth += 2;
      ...
      if (curwin->w_p_rl) {
        screen_fill(NULL, row, row + 1, pum_col - pum_base_width - n + 1,
                    col + 1, ' ', ' ', attr);
        col = pum_col - pum_base_width - n + 1;
      } else {
        screen_fill(NULL, row, row + 1, col, pum_col + pum_base_width + n,
                    ' ', ' ', attr);
    ...
    if (curwin->w_p_rl) {
      screen_fill(NULL, row, row + 1, pum_col - pum_width + 1, col + 1, ' ', ' ',
                  attr);
    } else {
      screen_fill(NULL, row, row + 1, col, pum_col + pum_width, ' ', ' ', attr);
    }

    if (pum_scrollbar > 0) {
      if (curwin->w_p_rl) {
        screen_putchar(NULL, ' ', row, pum_col - pum_width,
