...
static char_u *screen_puts_mbyte(char_u *s, int l, int attr)
{
  ...
  screen_puts_len(NULL, s, l, msg_row, msg_col, attr);
  if (cmdmsg_rl) {
    msg_col -= cw;
    if (msg_col == 0) {
      msg_col = Columns;
...
static void msg_scroll_up(void)
{
  if (dy_flags & DY_MSGSEP) {
    if (msg_scrolled == 0) {
      screen_fill(NULL, Rows-p_ch-1, Rows-p_ch, 0, (int)Columns,
                  fill_msgsep, fill_msgsep, hl_attr(HLF_MSGSEP));
    }
    int nscroll = MIN(msg_scrollsize()+1, Rows);
    screen_del_lines(NULL, Rows-nscroll, 1, Rows, 0, Columns);
  } else {
    screen_del_lines(NULL, 0, 1, (int)Rows, 0, Columns);
  }
}
...
static void t_puts(int *t_col, const char_u *t_s, const char_u *s, int attr)
{
  // Output postponed text.
  msg_didout = true;  // Remember that line is not empty.
  screen_puts_len(NULL, (char_u *)t_s, (int)(s - t_s), msg_row, msg_col, attr);
  msg_col += *t_col;
  *t_col = 0;
    ...
    if (toscroll != 0) {
      if (toscroll < 0) {
        ...
        if (mp != NULL && mp->sb_prev != NULL) {
          ...
          if (toscroll == -1
              && screen_ins_lines(NULL, 0, 1, (int)Rows,
                                  0, (int)Columns) == OK) {
            // display line at top
        ...
        while (toscroll > 0 && mp_last != NULL) {
          ...
          msg_scroll_up();
          inc_msg_scrolled();
          screen_fill(NULL, (int)Rows - 2, (int)Rows - 1, 0,
              (int)Columns, ' ', ' ', 0);
      ...
      if (toscroll <= 0) {
        ...
        screen_fill(NULL, (int)Rows - 1, (int)Rows, 0,
            (int)Columns, ' ', ' ', 0);
        msg_moremsg(FALSE);
        continue;
      }
...
{
  msg_didout = true;            // remember that line is not empty
  screen_putchar(NULL, c, msg_row, msg_col, attr);
  if (cmdmsg_rl) {
    if (--msg_col == 0) {
      msg_col = Columns;
      ++msg_row;
...
void msg_moremsg(int full)
{
  ...
  attr = hl_attr(HLF_M);
  screen_puts(NULL, s, (int)Rows - 1, 0, attr);
  if (full) {
    screen_puts(NULL, (char_u *)
...
{
  if (cmdmsg_rl) {
    screen_fill(NULL, msg_row, msg_row + 1, 0, msg_col + 1, ' ', ' ', 0);
    screen_fill(NULL, msg_row + 1, (int)Rows, 0, (int)Columns, ' ', ' ', 0);
  } else {
    screen_fill(NULL, msg_row, msg_row + 1, msg_col, (int)Columns, ' ', ' ', 0);
    screen_fill(NULL, msg_row + 1, (int)Rows, 0, (int)Columns, ' ', ' ', 0);
  }
}
